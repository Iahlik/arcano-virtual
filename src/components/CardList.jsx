...
import '../styles.css'; // Asegúrate de que este archivo tiene los estilos necesarios
import { jsPDF } from 'jspdf';
import 'jspdf-autotable';

const CardList = ({ arcanos, setSentences }) => {
    ...
    useEffect(() => {
        const shuffledArcanos = [...arcanos].sort(() => 0.5 - Math.random());
        const initialCards = shuffledArcanos.slice(0, 10).map(arcano => ({
            id: arcano.id,
    ...
    const handleCardClick = (index) => {
        if (index < 0 || index >= cards.length) {
            console.error('Carta no definida en el índice:', index);
            return;
        }

        if (!cards[index].revelado) {
            setCards(prevCards =>
                prevCards.map((c, i) =>
                    i === index ? { ...c, revelado: true } : c
                )
            );

            // Lógica para mostrar la sentencia dependiendo del arcano
            const cardId = cards[index].id;
            const sentence = sefirotSentences[cardId]?.[index];
            setSentences(prevSentences => {
                const newSentences = [...prevSentences];
                newSentences[index] = sentence ? sentence : "No hay sentencia definida para este arcano.";
                return newSentences;
            });
        }
    };
    ...
    const handleExportPDF = () => {
        ...
        doc.setFontSize(18);
        const title = 'Lista de Cartas y Significados';
        const titleWidth = doc.getTextWidth(title);
        const titleX = (doc.internal.pageSize.width - titleWidth) / 2; // Centrar título
        doc.text(title, titleX, 16);
    
        const tableData = cards.map((card) => [
            card.nombre,
            card.revelado ? sefirotSentences[card.id]?.[0] : 'Oculto'
        ]);
    
        doc.autoTable({
            head: [['Carta', 'Significado']],
            body: tableData,
            startY: 30, 
        });
       
        const contactText1 = '¿Te interesa profundizar en la tirada? Contáctame:';
        const contactText2 = '@tarotlaserena';
        const contactText3 = '+56942955496';
        const contactText4 = 'www.tarotlaserena.cl';
    
        const contactX1 = (doc.internal.pageSize.width - doc.getTextWidth(contactText1)) / 2;
        const contactX2 = (doc.internal.pageSize.width - doc.getTextWidth(contactText2)) / 2;
        const contactX3 = (doc.internal.pageSize.width - doc.getTextWidth(contactText3)) / 2;
        const contactX4 = (doc.internal.pageSize.width - doc.getTextWidth(contactText4)) / 2;
        
        doc.setFontSize(12);
        const startY = doc.lastAutoTable.finalY + 10; 
        doc.text(contactText1, contactX1, startY);
        doc.text(contactText2, contactX2, startY + 10);
        doc.text(contactText3, contactX3, startY + 20);
        doc.text(contactText4, contactX4, startY + 30);

        doc.link(contactX2, startY + 10 - 5, doc.getTextWidth(contactText2), 10, { url: 'https://instagram.com/tarot_laserena' });
        doc.link(contactX3, startY + 20 - 5, doc.getTextWidth(contactText3), 10, { url: 'tel:+56942955496' });
        doc.link(contactX4, startY + 30 - 5, doc.getTextWidth(contactText4), 10, { url: 'http://www.tarotlaserena.cl' });
    
        doc.save('tirada_tarot.pdf');
    };

    return (
        <div className="contenedor-arbol">
            <div className="card-grid">
                {cardPositions.map((row, rowIndex) => (
                    <div key={rowIndex} id={`row-${rowIndex + 1}`} className="card-row">
                        {row.map((item, columnIndex) => {
                            if (item && currentCardIndex < cards.length) {
                                const cardIndex = currentCardIndex;
                                currentCardIndex++;
                                return (
                                    <img
                                        key={`${rowIndex}-${columnIndex}`}
                                        src={cards[cardIndex].revelado ? cards[cardIndex].imagen : '/img/cardback.jpg'}
                                        alt={cards[cardIndex].revelado ? cards[cardIndex].nombre : 'Carta de Fondo'}
                                        onClick={() => handleCardClick(cardIndex)}
                                        className="card-image"
                                    />
                                );
                            } else {
                                return <div key={`${rowIndex}-${columnIndex}`} className="empty-space"></div>;
                            }
                        })}
                    </div>
                ))}
            </div>
            {cards.every(card => card.revelado) && (
                <div className="boton-impresion">
                    <button onClick={handleExportPDF}>Imprimir Tirada</button>
                </div>
            )}
        </div>
    );
